...
class JSONDataFormatter extends DataFormatter {
	 ...
	 * @return String XML
	 */
	public function convertDataObject(DataObjectInterface $obj, $fields = null, $relations = 1) {
		if(isset($obj->class)) {
		$className = $obj->class;
		$id = $obj->ID;
		...
		$dateCreated = date('d/m/Y h:i:s',strtotime($obj->Created));
		//$test = $dateCreated.format('d/m/Y');
		$jsonParts[] = "\"Created\" : \"" . $dateCreated . "\"";
		$jsonParts[] = "\"className\" : \"" . $className . "\"";
		if($relations < 0) {
			foreach($obj->has_one() as $relName => $relClass) {
				...
				if($obj->$fieldName) {
					...
					if($has_one_obj = DataObject::get_by_id($relClass, $obj->$fieldName)) {
						...
						$rtn = $f->convertDataObject($has_one_obj, null, ($relations + 1));
